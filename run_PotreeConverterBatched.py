...
if __name__ == "__main__":
    '''
    The run script for PotreeConverterMPI. 
    
    The main procedure is as follows (launched using the selected scheduler):

    * Read configuration, create directories, copy LAZ headers to temporary directory, and launch scheduler.
    ! TODO: Note that counting and indexing expects PotreeConverterBatched.lazPartitions to have been set.
    * Copy LAZ files to temporary storage (called counting).
    * Index LAZ files (actually distribution and indexing).
    ! "counting" copies the LAZ files from the input directory to the temporary input directory.
    ! "distribution" copies the LAZ files from the input directory to the temporary input directory.
    ! "indexing" sets the total batch size in the batch copier.
    * Wait for potreeConverterBatched to finish.
    ! How does this process progress?
    * Copy hierarchy.bin and metadata.json from the temporary output directory to the output directory.
    '''
    
    # Read the configuration file and launch the scheduler.
    if len(sys.argv) != 2:
